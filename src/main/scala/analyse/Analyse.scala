...
object Analyse {
  ...
  val spark = SparkSession
    ...
    val df = spark.read.option("header", "true").option("inferSchema","true")
      .csv("archived_nypd_msg/part*.csv")
    ...
    val df_infraction_annee = df.groupBy(col("code"),col("date")).count
    df_infraction_annee.show(5)
